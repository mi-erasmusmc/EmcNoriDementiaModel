...
#
# This file is part of EmcNoriDementiaModel
#
...
replaceName <- function(packageLocation = getwd(), 
  ...
  # rename files:
  #=====
  # file.path(packageLocation,"EmcNoriDementiaModel.Rproj")
  # file.path(packageLocation,"R/EmcNoriDementiaModel.R")
  filesToRename <- c("EmcNoriDementiaModel.Rproj","R/EmcNoriDementiaModel.R")
  for(f in filesToRename){
    ParallelLogger::logInfo(paste0('Renaming ', f))
    fnew <- gsub("EmcNoriDementiaModel", packageName, f)
  ...
  for( f in filesToEdit ){
    ...
    x <- readLines(f)
    y <- gsub( "EmcNoriDementiaModel", packageName, x )
    cat(y, file=f, sep="\n")
