getAnalyses <- function(settings, outputFolder,cdmDatabaseName){
  ...
  cohorts <- system.file("settings", 'CohortsToCreate.csv', package = "EmcNoriDementiaModel")
  cohorts <- read.csv(cohorts)
  
    settingLoc <- system.file("settings","settings.csv", package = "EmcNoriDementiaModel")
...
getPopulationSettings <- function(){
  ...
  runSettingsLoc <- system.file("settings", 'existingModelList.json', package = "EmcNoriDementiaModel")
